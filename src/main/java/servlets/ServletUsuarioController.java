...
/**
 * ve Servlet implementation class ServletUsuarioController
 */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (acao != null && !acao.isEmpty() && acao.equalsIgnoreCase("deleta")) {
				...
				List<ModelLogin> dadosJsonUser = daoUsuarioRepository.consultaUsuarioList(nomeBuscar,
				...
				response.addHeader("totalPagina", "" + daoUsuarioRepository
						.consultaUsuarioListTotalPaginaPaginacao(nomeBuscar, super.getUserLogado(request)));
				...
				List<ModelLogin> dadosJsonUser = daoUsuarioRepository.consultaUsuarioListOffSet(nomeBuscar,
				...
				if (dataInicial == null || dataInicial.isEmpty() && dataFinal == null || dataFinal.isEmpty()) {
					request.setAttribute("listaUser",
							daoUsuarioRepository.consultaUsuarioRel(super.getUserLogado(request)));
				} else {
					request.setAttribute("listaUser", daoUsuarioRepository
							.consultaUsuarioRel(super.getUserLogado(request), dataInicial, dataFinal));
				...
				if (dataInicial == null || dataInicial.isEmpty() && dataFinal == null || dataFinal.isEmpty()) {
					...
					modelLogins = daoUsuarioRepository.consultaUsuarioRel(super.getUserLogado(request));

				} else {

					modelLogins = daoUsuarioRepository.consultaUsuarioRel(super.getUserLogado(request), dataInicial,
							dataFinal);
				...
				if(acao.equalsIgnoreCase("imprimirRelatorioPDF")) {
					...
					relatorio = new ReportUtil().geraRelatorioPDF(modelLogins, "resl-user-jsp", params,
							request.getServletContext());
					extensao = "pdf";
				}else {
					if(acao.equalsIgnoreCase("imprimirRelatorioExcel")) {
						relatorio = new ReportUtil().geraRelatorioExcel(modelLogins, "resl-user-jsp", params,
								request.getServletContext());
						extensao = "xls";
					}
				}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (ServletFileUpload.isMultipartContent(request)) {
				...
				if (part.getSize() > 0) {
					byte[] foto = org.apache.commons.io.IOUtils
							.toByteArray(part.getInputStream()); /* converte imagem para byte */
